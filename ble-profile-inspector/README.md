...
Controls:
- **OK** starts an active scan session.
- **BACK** stops an active scan session.
- **BACK** again from idle exits the app.

On-screen output:
- Session status (idle/scanning)
- Scan run count
- Active advertisement count
- Up to 3 visual detail rows (name, RSSI, service UUID shorthand)
